 ...
 *     {@code groupId:artifactId} (e.g. {@code org.xwiki.platform:xwiki-platform-job-webjar}), where the {@code groupId}
 *     can be omitted if it is {@code "org.webjars"} (i.e. {@code angular} translates to {@code org.webjars:angular})
 * @param namespace the namespace in which the webjars resources will be loaded from (e.g. for a wiki namespace you
 *     should use the format {@code wiki:<wikiId>}). If null then defaults to the current wiki namespace. And if the
 *     passed namespace doesn't exist, falls back to the main wiki namespace
 ...
 *     parameters: {@code version} (the WebJar version) and {@code evaluate} (a boolean parameter that specifies if the
...
public record WebjarPathDescriptor(String webjarId, String namespace, String path, Map<String, ?> params)
{
     ...
     *     {@code groupId} can be omitted if it is {@code "org.webjars"} (i.e. {@code angular} translates to
    ...
    public WebjarPathDescriptor
    {
        ...
        if (webjarId.isBlank()) {
            throw new IllegalArgumentException("webjarId cannot be the blank");
        }
